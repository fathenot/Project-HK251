...
import { mdiMagnify, mdiPackageVariant, mdiDelete, mdiPencil } from '@mdi/js'
      ...
      <CardBox class="mb-6 rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div class="flex-1">
            ...
            <p class="text-sm text-gray-600">Tìm kiếm theo mã số, SKU hoặc tên hàng hóa</p>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <div class="space-y-2">
                ...
                <div v-if="selectedProduct.description" class="flex gap-4">
          ...
          <div class="space-y-4">
            ...
            <table class="w-full rounded border text-sm">
              <thead class="bg-gray-100">
                <tr>
                  <th class="w-20 px-1 py-2 text-center">Mã số</th>
                  <th class="w-64 px-1 py-2 text-center">Thuộc tính</th>
                  <th class="w-24 px-1 py-2 text-center">Thao tác</th>
                </tr>
              </thead>
              <tbody>
                <!-- Current Variants -->
                <tr v-for="v in selectedProduct.variants" :key="v.id" class="border-b">
                  <td class="w-20 p-2 text-center">{{ v.id }}</td>
                  <td class="w-64 p-2 text-center break-words">
                    <div v-for="(val, key) in v.variant_attributes" :key="key">
                      <strong>{{ key }}:</strong> {{ val }}
                    </div>
                  </td>
                  <td class="w-24 p-2 text-center">
                    <button
                ...
                <!-- New Variants -->
                <tr v-for="(row, index) in variantRows" :key="'new-' + index" class="border-b">
                  <td class="w-20 p-2 text-center">Mới</td>
                  <td class="w-64 p-2">
                    <input
                      v-model="row.key"
                      type="text"
                    ...
                    <input
                      ...
                      class="ml-2 w-28 rounded border border-gray-300 px-2 py-1 text-sm"
                    />
                  </td>
                  <td class="w-24 p-2 text-center">
                    <button
